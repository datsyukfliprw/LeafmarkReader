...
React + TypeScript + Vite PWA; Fastify; SQLite + Drizzle schema; IndexedDB offline persistence; TanStack Query; Zod; Vitest; Playwright; Bonsai 27B through an OpenAI-compatible local endpoint.
...
Open `http://YOUR-SERVER:8787` from Safari and add it to the iPad Home Screen. Set `APP_ORIGIN` to that exact origin first. See `docs/DEPLOYMENT.md` for the full service setup.

## AI configuration

By default Leafmark expects `http://127.0.0.1:1234/v1` and model name `Bonsai-27B`. Change `LOCAL_AI_BASE_URL`, `LOCAL_AI_MODEL`, and `LOCAL_AI_API_KEY` for your OpenAI-compatible local server. Normal generations are deliberately short. Model output is Zod-validated, timed out, and constrained by pedagogy-engine decisions.
...
`npm run backup` creates an integrity-checked SQLite backup in `BACKUP_DIR`. Copy backups to another physical device. Restore only while Leafmark is stopped: `RESTORE_FROM=/path/to/file.sqlite npm run restore`.

## Repository map

- `apps/web` child + parent PWA
- `apps/server` local API, auth, DB access, metadata adapter
